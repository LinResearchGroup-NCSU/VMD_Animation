#This is a modified version of the visualization.tcl 
#Function move_vp_render was modified to increment the output numbering of the pictures
#Function take_picture was modified to render the pictures in .png format and method TachyonInternal
...
proc take_picture {args} {
  ...
  if {$arg1 == "reset"} {
    ...
    set take_picture(format) "./animate.%04d.png"
...
proc move_vp {start end {morph_frames 50} args} {
  global viewpoints take_picture
  set pi 3.1415926535
  set smooth 1
  set tumble 0
  set ninja  0
  ...
  for {set j $firstj} {$j<= ($morph_frames - 1)} {incr j} {
    ...
    if {$render} {
      ...
      #incr framenum
      set frametext [format "%04d" $take_picture(frame)]
      set fileName [file join $::VCR::dirName "animate.$frametext.png"]
      render TachyonInternal $fileName
      #puts "Rendering frame $fileName"
      incr take_picture(frame); #modified original code to increment the output numbering of the pictures
    }
  }
}
...
proc move_vp_render {start end dirName {morph_frames 50} args} {
  global take_picture
  #set ::VCR::first_frame_num $first_frame_num
  set ::VCR::dirName         $dirName
  #et ::VCR::filePrefixName  $filePrefixName
  eval move_vp $start $end $morph_frames -render $args
}
